...
pkg_build_deps=(core/make core/gcc core/git core/patch core/rsync)
...
do_unpack() {
  ...
  build_line "Copying files to src cache"
  rsync -ar --cvs-exclude --exclude='tmp/cache' --exclude='results' --exclude='habitat' --exclude 'components' "$PLAN_CONTEXT"/../.. "$HAB_CACHE_SRC_PATH/$pkg_dirname/"
}
...
do_prepare() {
  ...
  set_runtime_env RAILS_ENV "$RAILS_ENV"
  set_buildtime_env RAILS_ENV "$RAILS_ENV"

  # have to do this because rails assets:precompile calls yarn install
  # and we can't fix the shebang in time
...
do_build() {
  ...
  pushd "$HAB_CACHE_SRC_PATH/$pkg_dirname/" > /dev/null
    ...
    build_line "Installing gems"
    bundle install --deployment
    fix_env_interpreter "bin"

    build_line "Compiling rails assets"
    bin/rails assets:precompile
    # We have to do this here because precompile calls yarn intall
    fix_env_interpreter "node_modules/.bin"
  popd
}
...
# your package.
do_install() {
  build_line "Copying app files to ${pkg_prefix}/src"
  mkdir "$pkg_prefix/src"
  pushd "$HAB_CACHE_SRC_PATH/$pkg_dirname/" > /dev/null
    rm config/master.key
    cp -r * $pkg_prefix/src
    cp -r .bundle $pkg_prefix/src

    build_line "Fix up ruby bins"
    for target in $pkg_prefix/src/bin/*; do
      wrap_ruby_bin $target
    done
  popd
}
...
wrap_ruby_bin() {
  ...
  cat <<EOF > "$pkg_prefix/bin/$file"
#!$(pkg_path_for core/bash)/bin/bash
set -e

export HOME=$pkg_svc_var_path
source $pkg_prefix/RUNTIME_ENVIRONMENT
source $pkg_svc_config_path/environment.sh

cd $pkg_prefix/src
exec "${bin}" \$@
EOF
  chmod -v 755 "$pkg_prefix/bin/$file"
}

fix_env_interpreter() {
  local binpath="$1"
  for target in $binpath/*; do
